...
def test_streaming_chat_tool_calls_are_reassembled_for_codex():
    ...
    assert items[0]["call_id"] == "call_shell_1"
    assert items[0]["name"] == "shell"
    assert items[0]["arguments"] == "{\"cmd\":\"pwd\"}"
...
def test_state_tracks_usage_and_rate_headers(tmp_path: Path):
    ...
    store.add_usage(
        "groq",
        "model",
        UsageDelta(input_tokens=10, output_tokens=4),
    )
    store.observe_headers(
        "groq",
        {
